...
static int si_siiPDO(uint16 slave, uint8 t, int mapoffset, int bitoffset);

static char* dtype2string(uint16 dtype, uint16 bitlen);

static int si_PDOassign(uint16 slave, uint16 PDOassign, int mapoffset, int bitoffset);

static char* otype2string(uint16 otype);

static char* access2string(uint16 access);

static char* SDO2string(uint16 slave, uint16 index, uint8 subidx, uint16 dtype);
...
void slaveinfo(int slave)
{
    ...
    else if( slave > 0 && slave <= ec_slavecount)
    {
        init = slave;
        end = slave;
    }
    ...
    {
        printf(" slave %d no encontrado \n", slave);
    }

    ec_readstate();
    for( cnt = init ; cnt <= end ; cnt++)
    {
        printf("\nSlave:%d\n Name:%s\n Output size: %dbits\n Input size: %dbits\n State: %d\n Delay: %d[ns]\n Has DC: %d\n",
